...
describe('InstallmentEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'installment.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_INSTALLMENT_ENTID JSON to run live')
      return
    }
    ...
    const installment_ref01_list = await installment_ref01_ent.list(installment_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_INSTALLMENT_ENTID']
