...
describe('PromotionDirect', async () => {
  ...
  test('direct-list-promotion', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-promotion', setup.live)) return
    if (skipIfMissingIds(t, setup, ["promotion01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LEARNWORLDS_TEST_PROMOTION_ENTID']
