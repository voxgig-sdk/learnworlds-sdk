...
describe('PromotionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'promotion.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_PROMOTION_ENTID JSON to run live')
      return
    }
    ...
    promotion_ref01_data = await promotion_ref01_ent.create(promotion_ref01_data)
    ...
    const promotion_ref01_data_dt0 = await promotion_ref01_ent.load(promotion_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_PROMOTION_ENTID']
