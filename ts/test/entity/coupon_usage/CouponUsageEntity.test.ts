...
describe('CouponUsageEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'coupon_usage.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_COUPON_USAGE_ENTID JSON to run live')
      return
    }
    ...
    const coupon_usage_ref01_list = await coupon_usage_ref01_ent.list(coupon_usage_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['coupon_usage01','coupon_usage02','coupon_usage03','promotion01','promotion02','promotion03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_COUPON_USAGE_ENTID']
