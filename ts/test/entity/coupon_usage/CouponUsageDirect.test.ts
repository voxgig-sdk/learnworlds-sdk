...
describe('CouponUsageDirect', async () => {
  ...
  test('direct-list-coupon_usage', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-coupon_usage', setup.live)) return
    if (skipIfMissingIds(t, setup, ["coupon_usage01","promotion01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LEARNWORLDS_TEST_COUPON_USAGE_ENTID']
