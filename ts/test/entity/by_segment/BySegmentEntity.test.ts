...
describe('BySegmentEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of []) {
      if (maybeSkipControl(t, 'entityOp', 'by_segment.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_BY_SEGMENT_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_BY_SEGMENT_ENTID']
