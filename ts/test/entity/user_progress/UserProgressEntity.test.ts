...
describe('UserProgressEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'user_progress.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_USER_PROGRESS_ENTID JSON to run live')
      return
    }
    ...
    const user_progress_ref01_list = await user_progress_ref01_ent.list(user_progress_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_USER_PROGRESS_ENTID']
