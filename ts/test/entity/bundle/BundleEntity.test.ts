...
describe('BundleEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'bundle.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_BUNDLE_ENTID JSON to run live')
      return
    }
    ...
    const bundle_ref01_list = await bundle_ref01_ent.list(bundle_ref01_match)


    // LOAD
    const bundle_ref01_match_dt0: any = {}
    bundle_ref01_match_dt0.id = bundle_ref01_data.id
    const bundle_ref01_data_dt0 = await bundle_ref01_ent.load(bundle_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_BUNDLE_ENTID']
