...
describe('SubscriptionPlanEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'subscription_plan.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_SUBSCRIPTION_PLAN_ENTID JSON to run live')
      return
    }
    ...
    const subscription_plan_ref01_list = await subscription_plan_ref01_ent.list(subscription_plan_ref01_match)


    // LOAD
    const subscription_plan_ref01_match_dt0: any = {}
    subscription_plan_ref01_match_dt0.id = subscription_plan_ref01_data.id
    const subscription_plan_ref01_data_dt0 = await subscription_plan_ref01_ent.load(subscription_plan_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_SUBSCRIPTION_PLAN_ENTID']
