...
describe('CourseContentDirect', async () => {
  ...
  test('direct-list-course_content', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-course_content', setup.live)) return
    if (skipIfMissingIds(t, setup, ["course_content01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LEARNWORLDS_TEST_COURSE_CONTENT_ENTID']
