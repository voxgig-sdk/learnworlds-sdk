...
describe('CourseContentEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list']) {
      if (maybeSkipControl(t, 'entityOp', 'course_content.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_COURSE_CONTENT_ENTID JSON to run live')
      return
    }
    ...
    course_content_ref01_data = await course_content_ref01_ent.create(course_content_ref01_data)
    ...
    const course_content_ref01_list = await course_content_ref01_ent.list(course_content_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_COURSE_CONTENT_ENTID']
