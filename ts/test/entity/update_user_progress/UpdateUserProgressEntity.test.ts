...
describe('UpdateUserProgressEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'update_user_progress.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_UPDATE_USER_PROGRESS_ENTID JSON to run live')
      return
    }
    ...
    update_user_progress_ref01_data = await update_user_progress_ref01_ent.create(update_user_progress_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_UPDATE_USER_PROGRESS_ENTID']
