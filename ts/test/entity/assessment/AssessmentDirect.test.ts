...
describe('AssessmentDirect', async () => {
  ...
  test('direct-list-assessment', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-assessment', setup.live)) return
    if (skipIfMissingIds(t, setup, ["form01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LEARNWORLDS_TEST_ASSESSMENT_ENTID']
