...
describe('AssessmentEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'assessment.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_ASSESSMENT_ENTID JSON to run live')
      return
    }
    ...
    const assessment_ref01_list = await assessment_ref01_ent.list(assessment_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_ASSESSMENT_ENTID']
