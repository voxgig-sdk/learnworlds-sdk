...
describe('CertificateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'update']) {
      if (maybeSkipControl(t, 'entityOp', 'certificate.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_CERTIFICATE_ENTID JSON to run live')
      return
    }
    ...
    const certificate_ref01_list = await certificate_ref01_ent.list(certificate_ref01_match)
    ...
    const certificate_ref01_resdata_up0 = await certificate_ref01_ent.update(certificate_ref01_data_up0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_CERTIFICATE_ENTID']
