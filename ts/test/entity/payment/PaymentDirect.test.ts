...
describe('PaymentDirect', async () => {
  ...
  test('direct-load-payment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-payment', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LEARNWORLDS_TEST_PAYMENT_ENTID']
