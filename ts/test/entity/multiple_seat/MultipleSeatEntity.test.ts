...
describe('MultipleSeatEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'remove']) {
      if (maybeSkipControl(t, 'entityOp', 'multiple_seat.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_MULTIPLE_SEAT_ENTID JSON to run live')
      return
    }
    ...
    multiple_seat_ref01_data = await multiple_seat_ref01_ent.create(multiple_seat_ref01_data)
    ...
    const multiple_seat_ref01_list = await multiple_seat_ref01_ent.list(multiple_seat_ref01_match)
    ...
    const multiple_seat_ref01_list_rt0 = await multiple_seat_ref01_ent.list(multiple_seat_ref01_match_rt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_MULTIPLE_SEAT_ENTID']
