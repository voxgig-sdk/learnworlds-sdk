...
describe('UserRoleEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'user_role.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_USER_ROLE_ENTID JSON to run live')
      return
    }
    ...
    const user_role_ref01_list = await user_role_ref01_ent.list(user_role_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_USER_ROLE_ENTID']
