...
describe('UnitAnalyticsDirect', async () => {
  ...
  test('direct-load-unit_analytics', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-unit_analytics', setup.live)) return
    if (skipIfMissingIds(t, setup, ["course_id01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LEARNWORLDS_TEST_UNIT_ANALYTICS_ENTID']
