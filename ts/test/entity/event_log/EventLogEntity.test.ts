...
describe('EventLogEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'event_log.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_EVENT_LOG_ENTID JSON to run live')
      return
    }
    ...
    const event_log_ref01_list = await event_log_ref01_ent.list(event_log_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_EVENT_LOG_ENTID']
