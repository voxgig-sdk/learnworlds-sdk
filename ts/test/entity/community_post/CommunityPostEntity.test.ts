...
describe('CommunityPostEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let community_post_ref01_data = Object.values(setup.data.existing.community_post)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/community_post/CommunityPostTestData.json')
  ...
  let idmap = transform(
    ['community_post01','community_post02','community_post03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
