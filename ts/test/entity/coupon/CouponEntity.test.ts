...
describe('CouponEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_COUPON_ENTID JSON to run live')
      return
    }
    ...
    coupon_ref01_data = await coupon_ref01_ent.create(coupon_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_COUPON_ENTID']
