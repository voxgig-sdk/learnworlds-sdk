...
describe('CourseEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set LEARNWORLDS_TEST_COURSE_ENTID JSON to run live')
      return
    }
    ...
    // UPDATE
    const course_ref01_data_up0: any = {}
    course_ref01_data_up0.id = course_ref01_data.id

    const course_ref01_markdef_up0 = { name: 'access', value: 'Mark01-course_ref01_' + setup.now }
    ;(course_ref01_data_up0 as any)[course_ref01_markdef_up0.name] = course_ref01_markdef_up0.value

    const course_ref01_resdata_up0 = await course_ref01_ent.update(course_ref01_data_up0)
    assert(course_ref01_resdata_up0.id === course_ref01_data_up0.id)

    assert((course_ref01_resdata_up0 as any)[course_ref01_markdef_up0.name] === course_ref01_markdef_up0.value)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['course01','course02','course03','user01','user02','user03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['LEARNWORLDS_TEST_COURSE_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'LEARNWORLDS_TEST_COURSE_ENTID': idmap,
    'LEARNWORLDS_TEST_LIVE': 'FALSE',
    'LEARNWORLDS_TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['LEARNWORLDS_TEST_COURSE_ENTID']
