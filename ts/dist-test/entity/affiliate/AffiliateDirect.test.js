...
(0, node_test_1.describe)('AffiliateDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-affiliate', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-affiliate', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["affiliate01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['affiliate01'];
        }
        else {
            params.id = 'direct01';
        }
        const result = await client.direct({
            path: 'v2/affiliates/{id}/customers',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LEARNWORLDS_TEST_AFFILIATE_ENTID'];
...
//# sourceMappingURL=AffiliateDirect.test.js.map