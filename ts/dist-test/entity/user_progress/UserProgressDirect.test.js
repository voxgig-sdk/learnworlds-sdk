...
(0, node_test_1.describe)('UserProgressDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-user_progress', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-user_progress', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["course01", "user01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.course_id = setup.idmap['course01'];
        ...
        const result = await client.direct({
            path: 'v2/users/{user_id}/courses/{course_id}/progress',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LEARNWORLDS_TEST_USER_PROGRESS_ENTID'];
...
//# sourceMappingURL=UserProgressDirect.test.js.map