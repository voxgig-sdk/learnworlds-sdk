...
(0, node_test_1.describe)('PaymentDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-payment', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-payment', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LEARNWORLDS_TEST_PAYMENT_ENTID'];
