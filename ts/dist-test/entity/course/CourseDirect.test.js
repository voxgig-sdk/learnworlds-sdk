...
(0, node_test_1.describe)('CourseDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-course', async (t) => {
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'v2/courses/{id}/grades',
                method: 'GET',
                params: {
                    id: setup.idmap['course01'],
                },
            });
    ...
    (0, node_test_1.test)('direct-list-course', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-course', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["course01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['course01'];
        }
        else {
            params.id = 'direct01';
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LEARNWORLDS_TEST_COURSE_ENTID'];
