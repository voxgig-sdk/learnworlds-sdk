...
(0, node_test_1.describe)('PromotionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-promotion', async (t) => {
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'v2/promotions/{id}/coupons',
                method: 'GET',
                params: {
                    id: setup.idmap['promotion01'],
                },
            });
    ...
    (0, node_test_1.test)('direct-list-promotion', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-promotion', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["promotion01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['promotion01'];
        }
        else {
            params.id = 'direct01';
        }
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LEARNWORLDS_TEST_PROMOTION_ENTID'];
