...
|---|---|---|
| `seneca-provider/` | voxgig-sdk-learnworlds | `.github/workflows/publish-seneca-provider.yml` |
| `ts/` | @voxgig-sdk/learnworlds | `.github/workflows/publish-ts.yml` |

## How a release happens

The worked example below uses the `seneca-provider` target; every npm
...
1. Bump the version in the model, then regenerate:
       ...
       main: kit: target: seneca-provider: publish: version: '<x.y.z>'

2. Commit and push to `main`, and let CI go green.
3. Run the workflow from the Actions tab, or:

       gh workflow run publish-seneca-provider.yml --ref main -f expect_sha=$(git rev-parse HEAD)
...
| target | tag |
|---|---|
| `seneca-provider/` | `seneca-provider/v<version>` |
| `ts/` | `v<version>` |
...
required):
    ...
    npm trust github voxgig-sdk-learnworlds \
      --repository voxgig-sdk/learnworlds-sdk \
      --file publish-seneca-provider.yml \
      --allow-publish

Then `npm trust list voxgig-sdk-learnworlds` shows it, and
`npm trust revoke voxgig-sdk-learnworlds --id=<id>` removes it.

**The workflow filename is part of the configuration.** Renaming
`publish-seneca-provider.yml` breaks publishing until the npm side is
updated to match.
